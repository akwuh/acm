...
int main()
{
	...
	AdjacencyList al(n * n);
	
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < n; ++j) {
			if (a[i][j] == '@' && start == -1) start = i * n + j;
			if (a[i][j] == '@' && start != -1) end = i * n + j;
			if (a[i][j] == '#') continue;
			if (i > 1 && j > 0 && a[i - 2][j - 1] != '#') al.push(i * n + j, (i - 2) * n + (j - 1), 1);
			if (i > 1 && j < n - 1 && a[i - 2][j + 1] != '#') al.push(i * n + j, (i - 2) * n + (j + 1), 1);
			if (i < n - 2 && j > 0 && a[i + 2][j - 1] != '#') al.push(i * n + j, (i + 2) * n + (j - 1), 1);
			if (i < n - 2 && j < n - 1 && a[i + 2][j + 1] != '#') al.push(i * n + j, (i + 2) * n + (j + 1), 1);
			if (i > 0 && j > 1 && a[i - 1][j - 2] != '#') al.push(i * n + j, (i - 1) * n + (j - 2), 1);
			if (i < n - 1 && j > 1 && a[i + 1][j - 2] != '#') al.push(i * n + j, (i + 1) * n + (j - 2), 1);
			if (i > 0 && j < n - 2 && a[i - 1][j + 2] != '#') al.push(i * n + j, (i - 1) * n + (j + 2), 1);
			if (i < n - 1 && j < n - 2 && a[i + 1][j + 2] != '#') al.push(i * n + j, (i + 1) * n + (j + 2), 1);
		}
	}
